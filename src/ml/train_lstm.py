...
def train():
    ...
    # 1. READ GOLD FROM S3
    logging.info("Reading Gold Data from MinIO...")
    try:
        df = spark.read.format("delta").load("s3a://finance-lake/lake/gold")
    except Exception as e:
        logging.error(f"Could not read Gold data. Is the pipeline running? Error: {e}")
        return
    ...
    logging.info(f"Materializing {df_seq.count()} samples to {local_train_dir}...")
    ...
    with mlflow.start_run():
        ...
        logging.info("Model trained and saved to MLflow.")
